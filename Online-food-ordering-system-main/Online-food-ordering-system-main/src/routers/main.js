...
const fs = require('fs')
const Review = require('../modul/review'); // Adjust the path if necessary
...
route.post('/saveDish', async (req, res) => {
    ...
    photo.mv("C:\Users\KEESHA LUTHRA\OneDrive\Desktop\AlumHack\Online-food-ordering-system-main (1)\Online-food-ordering-system-main\public\dishImage" + imageName);
    if (data) {
...
route.get('/getCounts', async (req, res) => {
    try {
        const userCount = await User.countDocuments();  // Fetch the total number of users
        const orderCount = await order.countDocuments();  // Fetch the total number of orders
        const dishCount = await dish.countDocuments();  // Fetch the total number of dishes
...
// Submit a new review
route.post('/submitReview', async (req, res) => {
    if (req.session.loginUser) {
        ...
        if (wordCount > 500) {
            return res.render('index', {
                ...
                reviewError: 'Review must be under 50 words.'
            });
        }
        try {
            const newReview = await Review.create({
                userId: req.session.loginUser._id,
                userName: req.session.loginUser.name, // Assuming 'name' is a field in your User model
                reviewText: reviewText
                // dishId: req.body.dishId // Include if tying to specific dishes
            });
            res.render('index', {
                loginUser: req.session.loginUser,
